...
### ISS-02: `handleSubscribe` has no membership check
...
### ISS-03: `handleTyping` has no membership check
...
### ISS-04: TOCTOU race on invite use-count
...
### ISS-05: OTP key claim is not atomic
...
### ISS-06: Double-Identify causes session duplication
...
### ISS-07: `App.tsx` inline restore never sets `user`
...
### ISS-08: `authStore.restore()` is never called
...
### ISS-09: Heartbeat timeout never evicts dead sessions
...
### ISS-10: `sendPayload` doesn't check socket readyState
...
### ISS-11: `useAuthStore()` without selector in UserArea
...
### ISS-12: `getChannelMessages` and `getSpaceChannels` use `|| []`
...
### ISS-13: Token stored in localStorage — XSS exposure
...
### ISS-14: No `MAX_MESSAGE_SIZE` check on WS messages
...
### ISS-15: No size limit on REST message edit
...
### ISS-16: Delete button visible to all users
...
### ISS-17: `fetchedRef` prevents channel refresh on space re-entry
...
### ISS-18: `gateway.connect()` called twice on fresh login
...
### ISS-19: Message action buttons clip on first message
...
### ISS-20: Edit textarea doesn't sync when message updates externally
...
### ISS-21: Channel header shows truncated ID, not channel name
...
### ISS-22: `typingState` map never cleaned proactively
...
### ISS-23: Typing timeout not cleared on unmount
...
### ISS-24: WS identify doesn't update `last_used_at`
...
### ISS-25: Reconnect timer silently canceled by cleanup
...
### ISS-26: `require("fs")` in ESM context
...
### ISS-27: Rate limiter per path+IP, not per IP alone
...
### ISS-28: `c.req.json()` parse errors return 500
...
### ISS-29: Identity key is random bytes, not a real ECDH key pair
...
### ISS-30: Space delete doesn't clean up `read_states` or `mls_groups`
...
### ISS-31: Channel delete doesn't clean up `read_states`
...
### ISS-32: Missing database indexes
...
### ISS-33: `StrictMode` imported but not applied
...
### ISS-34: `TIMESTAMP_SHIFT` hard-coded in client instead of shared
...
### ISS-35: `SESSION_TOKEN_LENGTH` naming is misleading
...
### ISS-36: `signedPrekey` never sent during registration
...
### ISS-37: `createRoot(document.getElementById("root")!)` non-null assertion
